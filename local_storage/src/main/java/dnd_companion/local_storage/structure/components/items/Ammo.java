...
public class Ammo extends GenericItem
{
	public Ammo retrieve(String name) throws InvalidOptionException {
		Ammo[] options = DataHandler.retrieve_options(new DataKey(AmmoTypeData.class));
		Optional<Ammo> option = Arrays.stream(options).filter(o -> o.name().equals(name)).findFirst();
		if (option.isEmpty()) {
			throw new InvalidOptionException("Invalid ammo name (" + name + ")");
		}
		Ammo ammo = option.get();
		return new Ammo(
			ammo.name(),
