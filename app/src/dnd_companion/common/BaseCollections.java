...
public enum BaseCollections implements Singleton
{
	DATA,
	...
	private static HashMap<BaseCollections, BaseCollections> mapping = new HashMap<>();
	public static HashMap<BaseCollections, BaseCollections> mapping() {return mapping;}

	private static void add(BaseCollections parent, BaseCollections collection) {
		mapping.put(collection, parent);
	}

	public String path() {
		String path = this.name().toLowerCase();
		BaseCollections next = mapping.get(this);
		while(next != null) {
